...
class Upgrade:
    ...
    def purchase(self, cookies):                    # NEW
        ...
        if cookies >= self.price:                   # NEW
            # increase the quantity by 1            # NEW
            self.quantity += 1                      # NEW
            # increase the price by 15%             # NEW
            self.price = int(self.price * 1.15)     # NEW
            # return True                           # NEW
            return True                             # NEW
        # else                                      # NEW
        else:                                       # NEW
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if grandma.clicked(event.pos):
                # if the upgrade was purchased
                if grandma.purchase(total_cookies):
                    # subtract the price from the cookies
                    total_cookies -= grandma.price
                    # increase the quantity by 1
                    cookies_per_second += grandma.value
    ...
    total_cookies += cookies_per_second / 60    # NEW
    pygame.display.flip()
